...
class  Database:
    ...
    def set_personalities(self, user_id, personalities):
        with self.connection:
            return self.cursor.execute("UPDATE `users` SET `personalities` = ? WHERE `user_id` = ?", (personalities, user_id,))
        
    def get_personalities(self, user_id):
        with self.connection:
            result = self.cursor.execute("SELECT `personalities` FROM `users` WHERE `user_id` = ?", (user_id,)).fetchall()
            for row in result:
                personalities = str(row[0])
